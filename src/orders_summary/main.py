...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    """
    parser = argparse.ArgumentParser(description="Summarize orders by region")
    parser.add_argument("--input-path", required=True, help="Delta table location to read orders from")
    parser.add_argument("--output-path", required=True, help="Delta table location to write the summary to")
    return parser


def run(input_path: str, output_path: str) -> None:
    """Reads orders from a Delta table, summarizes them, and writes the result.

    Args:
        input_path: Delta table location to read orders from.
    ...
    """
    builder = (
        SparkSession.builder.appName("orders-summary")
        .config("spark.sql.extensions", "io.delta.sql.DeltaSparkSessionExtension")
        .config("spark.sql.catalog.spark_catalog", "org.apache.spark.sql.delta.catalog.DeltaCatalog")
    )
    spark = configure_spark_with_delta_pip(builder).getOrCreate()
    try:
        orders_df = spark.read.format("delta").load(input_path)
        summary_df = summarize_orders_by_region(orders_df)
